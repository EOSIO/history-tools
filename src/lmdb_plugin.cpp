...
void lmdb_plugin::set_program_options(options_description& cli, options_description& cfg) {
    ...
    op("lmdb-set-db-size-mb", bpo::value<uint32_t>(),
...
void lmdb_plugin::plugin_initialize(const variables_map& options) {
    try {
        auto size = options.count("lmdb-set-db-size-mb") ? options["lmdb-set-db-size-mb"].as<uint32_t>() : 0;
        my->db    = std::make_shared<lmdb_inst>(size);
        auto x    = read_string(options["lmdb-query-config"].as<std::string>().c_str());
        try {
            abieos::json_to_native(my->db->query_config, x);
            my->db->query_config.prepare(state_history::lmdb::abi_type_to_lmdb_type);
...
std::shared_ptr<lmdb_inst> lmdb_plugin::get_db() { return my->db; }
