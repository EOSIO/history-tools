...
inline std::string sql_str(bool bulk, const abieos::int128& v)                          { std::array<uint8_t, 128/8> t; auto nv = -v; return v < 0 ? std::string("-") + abieos::binary_to_decimal(*(std::array<uint8_t, 128/8>*)std::memcpy(&t, &nv, sizeof(uint8_t)*128/8)) : abieos::binary_to_decimal(*(std::array<uint8_t, 128/8>*)std::memcpy(&t, &v, sizeof(uint8_t)*128/8)); }
inline std::string sql_str(bool bulk, const abieos::uint128& v)                         { std::array<uint8_t, 128/8> t; std::memcpy(&t, &v, sizeof(uint8_t)*128/8); return abieos::binary_to_decimal(t); }
#else
