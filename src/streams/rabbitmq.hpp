...
class rabbitmq : public stream_handler {
 ...
 public:
   rabbitmq(boost::asio::io_service& io_service, std::vector<abieos::name> routes, std::string address, std::string name)
...
inline void initialize_rabbits(boost::asio::io_service&                      io_service,
   ...
   for (std::string rabbit : rabbits) {
       ilog("try to init rabbit: ${r}",("r",rabbit));
      ...
      std::vector<abieos::name> routings = {};
      if (has_router) {
